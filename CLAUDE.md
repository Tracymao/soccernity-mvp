...
- **`sprint-1/guardian-consent-decline-withdraw-expiry` (backend-api,
  ...
  was no way for a guardian to decline, no way to withdraw consent once
  given, and — despite DPIA finding R5 adding
  `Guardian.consentTokenExpiresAt` and a manual resend endpoint back in
  PR #42 — **nothing anywhere ever read that expiry** (confirmed by grep
  before any code was written). An unanswered request therefore left a
  minor's account restricted-pending indefinitely, holding a child's
  personal data with no resolution path. `services/api` only; no Figma,
  no `apps/web`. **Resolves Decision Log #34** (its backend half);
  Decision Log **#337–#340** added.** Full detail:
  `services/api/src/modules/auth/README.md`'s matching "Status update"
  entry.
  - **Three new endpoints**, all following `confirmConsent`'s existing
    token-is-the-credential trust model (no guard — the guardian holds no
    Soccernity account) and all `@AuthRateLimit()`:
    `POST /auth/guardian-consent/decline` `{consentToken}` (pending
    requests only); `POST /auth/guardian-consent/withdraw/request`
    `{email}` → issues a fresh single-use `Guardian.withdrawalToken`
    emailed **to the guardian**; `POST /auth/guardian-consent/withdraw`
    `{withdrawalToken}`. Plus **`GuardianConsentExpirySweepService`**
    (`@Cron(EVERY_HOUR)`), the job that finally acts on
    `consentTokenExpiresAt`. All four paths converge on one primitive,
    `refuseConsentAndScheduleDeletion()`, so the resulting account state
    cannot differ between them.
  - **First-vs-second expiry is `Guardian.consentAutoResentAt`, a new
    nullable timestamp — deriving it from existing fields was
    investigated and rejected as genuinely unsafe, not merely inelegant.**
    `consentTokenExpiresAt − TTL` approximates the issue time, so
    comparing against `User.createdAt` would reveal whether the token had
    ever rotated — but that breaks silently if
    `GUARDIAN_CONSENT_TOKEN_TTL_HOURS` changes, and cannot tell the
    platform's *automatic* re-send from a *manual* one the minor
    triggered via `/resend`. A minor who had manually re-sent once would
    look "already chased", so the next lapse would schedule their account
    for deletion **having never sent the automated warning at all**.
    Deliberately a timestamp, not an `Int` counter (a counter implies an
    N-reminders policy that doesn't exist). Defaults to `NULL`, so every
    Guardian row already in flight at deploy gets its one automatic
    re-send — no existing pending account is fast-tracked to deletion.
  - **Two-stage escalation.** First expiry → one automatic re-send
    through `GuardianConsentService.reissueConsentToken()` (the *exact*
    method the manual resend uses, extracted out of `resendConsent()`
    rather than copied; `markAutoResent` is passed explicitly by both
    callers so neither can acquire the other's behaviour) plus an email
    warning the **minor** that a second lapse closes the account. Second
    expiry → implicit decline. ~6 days end to end on the default 72h TTL.
    **Hourly, not daily** like `AccountDeletionSweepService`: the stages
    chain, so a daily tick would stretch a 72h+72h policy to ~8 days and
    make the real deadline depend on what time of day someone registered.
  - **Reuses the deletion machinery, invents no new state.**
    `AuthService.startPendingDeletion()` made **public** for this third
    caller — same precedent as `hardDeleteUser` being made public for
    `AdminUsersService`; that method's own comment asserts it is "the
    single place `accountStatus` flips to `pending_deletion`", an
    invariant a parallel `user.update` here would have broken. From there
    the ordinary Decision Log #42/#44 path applies unchanged. Two
    load-bearing details: the refusal is deliberately **not** a
    transaction (the writes span Postgres and Redis; the *order* is what
    makes partial failure safe, since the Guardian row is refused first
    and every enforcement site is an allowlist), and it **skips**
    `startPendingDeletion` when the account is already `pending_deletion`
    — re-running it would reset `pendingDeletionAt` and push an existing,
    earlier deadline out, so a guardian refusing consent could otherwise
    *extend* how long the platform holds a minor's data.
  - **One necessary, flagged change to `confirmConsent`** (the brief said
    not to touch it): `Guardian.consentToken` is `NOT NULL @unique` so it
    keeps resolving after a decline, and confirm's old
    `{ not: 'confirmed' }` write-guard would have flipped a **declined**
    row to confirmed — silently reversing a refusal on an account already
    scheduled for deletion. Added an explicit `declined` branch (a clear,
    non-generic error — the caller already holds the real token, so
    nothing is left to enumerate) and tightened the guard to the positive
    `'pending'` as the race-safe backstop. Exactly equivalent for every
    pre-existing case.
  - **Safeguarding audit: every `consentStatus` reader was grepped, not
    assumed.** All five enforcement sites are **allowlists**
    (`=== 'confirmed'`), never denylists — `guardian-consent.guard.ts`,
    `clubs.service.ts`, `community-groups.service.ts`,
    `messaging.service.ts`, `users.service.ts` — so `declined` fails safe
    everywhere with zero changes to any of them. That audit also found
    **two comments this PR makes factually wrong, both corrected in place
    rather than left to rot**: `contest.service.ts` justified its
    no-minor-filter admin read surface on "consentStatus only ever moves
    pending -> confirmed (no reversal path anywhere in the codebase)"
    (now false, with a real bounded consequence — Decision Log #339), and
    `messaging.service.ts` claimed its `GuardianConsentGuard` "can never
    actually block anyone here in practice" (it now can, and blocking is
    correct, needing no code change precisely because the guard is an
    allowlist). `registration.service.ts` had already *anticipated* this
    exact change and typed `guardianConsentStatus` as a plain `string`
    for it, so `POST /auth/verify-email` returns `'declined'` correctly
    with no type change needed.
  - **Decision Log candidates raised, not silently resolved**: **#337**
    (the three new `Guardian` fields, a flagged addition beyond Section
    3); **#338** — adding `declined` needed **no** column migration (it
    is already free-form `String`, matching six sibling string-enum
    columns), but a CHECK constraint is recommended for this one
    safeguarding column (**not** a Prisma enum), and all three refusal
    ...
    **#340** (the three routes as a genuine spec-gap addition, and why
    ...
    be resolved from their own address, and accepting it would open a new
    enumeration surface).
  - **Deliberately not built**: any guardian-facing web page — same
    API-contract-only scoping the original guardian-consent PR used.
    Worth knowing for the frontend ticket: `apps/web`'s
    `GuardianConsentConfirmPage.tsx` models the Figma frame's "I do not
    consent" button as "take no action, with an acknowledgement message"
    **precisely because no decline endpoint existed** — it can now be
    wired to a real action.
  - **Verification, real before/after, measured by stashing the branch
    and re-running against a clean checkout**: mocked suite **83 suites /
    1099 tests → 85 suites / 1139 tests, 0 failures** (two new spec files,
    19 + 11, plus 10 new controller cases); e2e suite (real Postgres via
    docker-compose) **19 suites / 185 tests → 20 suites / 193 tests, 0
    failures** (exactly one spec file added, **no existing e2e file
    modified** — confirmed via `git status` — so the before figure is
    exact, and it matches the last figure recorded on `main`).
    ...
    is purely additive (three nullable columns + two indexes, zero
    `ALTER`/`DROP`) and was applied and re-verified with `psql \d
    "Guardian"` against the real dev database, and by `global-setup`
    against `soccernity_test`. **`User`/`Guardian` safeguarding fields
    untouched.** *Honest note on the baseline:* the first clean-checkout
    run reported 2 failures, both real-HTTP/real-app-bootstrap specs
    under heavy contention (387s wall clock vs 47s on a quiet re-run,
    which was 83/1099 green) — the documented contention-flake class this
    repo already raised `testTimeout` to 30000ms for, not a real baseline
    failure.
  - **All three paths the brief asked to be traced — decline, withdrawal,
    double-expiry — have a real-Postgres e2e test** proving the account
    lands in `pending_deletion` *and* that the pre-existing
    `AccountDeletionSweepService` then picks it up on its normal 30-day
    terms and hard-deletes it, `ConsentAuditRecord` snapshot and all.
    Also proven: the 29-day/31-day grace boundary; that a withdrawal link
    is genuinely single-use (which depends on real Postgres NULL
...
- **`sprint-1/under-16-restrictions` (backend-api, 2026-09-20) builds counsel's under-16 tier from the ToS/Privacy review — `services/api` only, no Figma/`apps/web`. Decision Log #346.** A NEW tier layered ON TOP of `isMinor`/`GuardianConsentGuard`, never replacing it (an under-16 with confirmed consent is still restricted). New `User.isUnder16` (migration `20260920100000_add_user_is_under_16`, backfilled from `dateOfBirth`; set in `RegistrationService` beside `isMinor` via `computeIsUnder16`). New `Under16RestrictionGuard` + `@RestrictUnder16(feature)` (fresh Postgres read, 403 `code: under_16_restricted`, distinct from `guardian_consent_pending`, fails closed if untagged). Messaging: whole `/conversations` controller blocked for the sender; the recipient side is `MessagingService.assertRecipientMessageable` (403, not the 404 restricted-pending gets — confirms the account exists, flagged). Bants: whole `/banter-rooms` controller blocked, reads included. Community Groups: `POST /community-groups` blocked; reads and join/leave stay open (no group post/comment endpoint exists to gate). `GET /users/:id` (self-only) returns `isUnder16` and `guardianContact: { label: 'Guardian contact', email }` for an under-16 only; no list/roster shape selects Guardian. **Open, flagged in #346:** `isUnder16` is stored like `isMinor`, so it does not age out at 16 (needs a job); frontend does not yet handle `under_16_restricted`; "anyone can reach the guardian" read as own-profile exposure only. The 16–17 tier is a separate later PR. Verification: mocked 86 suites / 1172 tests, 0 failures (baseline derived as 85/1162: +1 suite, +10 tests); e2e 21 suites / 221 tests, 0 failures (+1 suite `under-16.e2e-spec.ts`, +4 tests; no existing e2e fixture conflicted — seeded minors default `isUnder16: false`); `tsc`, lint, `nest build` clean. Existing controller HTTP specs (messaging/banter/community-groups) needed a pass-through `overrideGuard(Under16RestrictionGuard)`. PR opened, not merged.
- **`sprint-1/adult-to-minor-dm-block` (backend-api, 2026-09-20) adds counsel's data-model-level rule that an adult cannot initiate a DM with a minor — `services/api` only. Decision Log #347.** `MessagingService.startConversation` now 403s (`code: adult_to_minor_dm_blocked`) when the caller is `isMinor: false` and the recipient `isMinor: true` AND no thread exists yet. An existing thread is returned unchanged (200) and message-send into it is unaffected; minor->adult, minor->minor, adult->adult allowed. No prior connection/follow-back gate existed. Open: `isMinor` is stored not recomputed (no age-out at 18); frontend doesn't handle the code. Verification: mocked 86 suites / 1172 -> 86 / 1175 tests, e2e (real Postgres) 21 / 221 -> 21 / 222, 0 failures; tsc, lint, nest build clean. One existing e2e positive control (adult -> confirmed minor) was switched to minor -> minor. PR opened, not merged.
- **`sprint-1/consent-audit-screen-version-device-type` (backend-api, 2026-09-20) adds counsel's UK GDPR Art. 7(1) evidence fields — `services/api` only. Decision Log #348.** `ConsentAuditRecord` gains nullable `consentScreenVersion` + `deviceType`, but it only exists from anonymization time (a snapshot), so both are stamped onto `Guardian` (new nullable `consentScreenVersion` / `consentDeviceType`) in the SAME `updateMany` as `consentTimestamp` in `confirmConsent` (no new write path), then snapshotted by `AccountDeletionSweepService`. Migration `20260920120000_...` is purely additive, no backfill. **`consentScreenVersion` is a MANUALLY BUMPED constant** (`CONSENT_SCREEN_VERSION = 'v1'`, `consent-screen-version.constants.ts`) — nothing derives or verifies it; bump it in any PR that materially changes the guardian-consent confirmation screen's wording. `deviceType` is `mobile|desktop|unknown` from the User-Agent (`device-type.util.ts`); the raw UA is never stored. An idempotent re-confirm never overwrites the original evidence. No admin/compliance export of `ConsentAuditRecord` exists to update. Verification (before -> after): mocked 86 suites / 1175 -> 87 / 1188 tests; e2e (real Postgres) 21 / 222 -> 22 / 225 (new `guardian-consent-audit-evidence.e2e-spec.ts`; existing exact-key assertion in the sweep e2e updated); `tsc`, lint, `nest build` clean. PR opened, not merged.
- **`sprint-1/age-reclassification-sweep` (backend-api, 2026-09-20) closes the age-out gap Decision Log #346/#347 disclosed — `services/api` only. Decision Log #349.** New `AgeReclassificationSweepService` (`@Cron(EVERY_DAY_AT_2AM)`, module `age-reclassification`) recomputes `User.isMinor` / `isUnder16` from `dateOfBirth` (same `age.util` functions as registration) for mismatched accounts, both directions, skipping DOB-less anonymized rows; one append-only `AgeReclassificationLog` row per field changed (plain-string `userId`, no FK, derived age not DOB), same transaction as the update (migration `20260920140000_add_age_reclassification_log`, additive). **Immediate, not next-login**: the JWT carries only `{sub, role}` and `GuardianConsentGuard`, `Under16RestrictionGuard` and the messaging checks all read fresh from Postgres — proven by e2e using the SAME pre-sweep token. **Turning 18 hardening**: Guardian rows are kept (consent history), but the consent-expiry sweep and all refusal paths (decline/withdraw/expiry) now skip non-minors so a stale pending request can't auto-decline an adult into deletion; `anonymizeUser` snapshots any Guardian row regardless of `isMinor`. Open: daily cadence (~24h lag), no user/guardian notification, no frontend change. Verification: mocked 87 suites / 1188 (1 failure in that baseline run, unidentified contention flake) -> 88 / 1194, 0 failures (+1 suite, +6 tests; 2 existing assertions updated); e2e 22 / 225 (last recorded figure) -> 23 / 230 (+`age-reclassification.e2e-spec.ts`, 5 tests); `tsc`, lint, `nest build` clean. PR opened, not merged.
- **`sprint-1/age-reclassification-notifications` (backend-api, 2026-09-20) makes the age-reclassification sweep (Decision Log #349) tell someone when it acts — `services/api` only, zero schema migration.** Turning 18 (`isMinor` true→false): the guardian on file gets a plain informational email (`sendGuardianMinorTurned18Email`, new `guardian-minor-turned-18` template on the existing `RegistrationEmailService`/Postmark path; asks nothing of them; no Guardian row → nothing sent). Turning 16 (`isUnder16` true→false): the user gets an in-app `Notification` of new type `age_milestone` (`payloadRefId: 'under_16_lifted'`; `NotificationsService` resolves it to `{ milestone }`; `Notification.type` comment updated). Both are best-effort and run strictly AFTER the reclassification transaction commits, each in its own try/catch, so a failure can never roll back or block the flag update. The younger direction (false→true) sends nothing, is `logger.warn`ed distinctly as a likely corrected DOB, and is returned in the new `reclassifiedYoungerUserIds`. **Not built / flagged:** `apps/web`'s Notification Centre has no renderer for `age_milestone` yet, so it currently shows the generic "no longer available" row (figma-to-code + design follow-up); the email body is functional placeholder copy, not counsel-reviewed. Verification (before -> after): mocked 89 suites / ~1200 (derived from last recorded + merged tripwire spec, not re-run) -> 89 / 1205, 0 failures; e2e (real Postgres) 23 suites / 230 -> 23 / 233, 0 failures (one cold-start `grassroots` hook timeout in the full run, passed 31/31 re-run alone); `tsc`, lint, `nest build` clean.
- **`sprint-1/adult-to-minor-dm-enumeration-fix` (backend-api, 2026-09-20) closes the info-disclosure gap Decision Log #347 disclosed — `services/api` only.** An adult starting a NEW conversation with a confirmed minor now gets the exact same 404 (`User not found`, same class/body) as a non-existent/restricted-pending/deactivated recipient, replacing the distinct 403 `adult_to_minor_dm_blocked` (constant removed). The block itself is unchanged; only what the response reveals. Minor->adult, minor->minor, adult->adult and adult resuming an existing thread unchanged. Unit + two e2e assertions rewritten (e2e asserts body equality with a real missing-id 404). Verification: mocked 88 suites / 1194 tests and e2e 23 suites / 230 tests, 0 failures, counts identical to before (assertions rewritten in place); tsc, lint, nest build clean. **Still open, flagged:** the under-16 recipient 403 (`under_16_restricted`) still confirms the account exists (already disclosed in #346); the blocked path does one extra `user.findUnique` vs the 404 paths (sub-ms-to-few-ms DB timing delta, not equalised). PR opened, not merged.
- **`sprint-1/consent-screen-version-tripwire` (backend-api, 2026-09-20) adds a Jest tripwire for the manually bumped `CONSENT_SCREEN_VERSION` (Decision Log #348) — `services/api` only.** `consent-screen-version.spec.ts` hashes the normalised guardian-visible wording of `GuardianConsentConfirmPage.tsx` (JSX text + top-level const strings, via the TypeScript AST; case/whitespace/punctuation ignored) and compares it to `CONSENT_SCREEN_TEXT_HASH` beside the version constant; on mismatch it fails telling the author to bump the version and update the hash in the same PR. Deliberately not an auto-bumper. A one-letter typo fix DOES trip it (documented false positive: update only the hash). PR opened, not merged.
- **`sprint-1/dm-enumeration-timing-parity` (backend-api, 2026-09-20) closes the timing side channel left by `sprint-1/adult-to-minor-dm-enumeration-fix` — `services/api` only, no schema change.** `MessagingService.startConversation` now does exactly the same two reads for every attempt, in the same order: one `user.findMany` (caller + recipient, recipient's `guardian.consentStatus` joined in) and one `conversation.findUnique`; missing / deactivated / restricted-pending / adult->confirmed-minor are then decided in memory (`assertRecipientMessageable` is now pure and synchronous). Before, the paths did 1 / 1 / 2 / 4 queries. No artificial delay. New unit test asserts identical query counts (1 user read, 1 conversation read, 0 `user.findUnique`, 0 `guardian.findUnique`) and byte-identical read arguments across all four 404 outcomes. Existing behaviours (under-16 403, existing-thread resume, minor->adult/minor->minor) unchanged. **Still open:** the successful path and the under-16 403 are distinguishable by design; DB latency jitter itself is not eliminated, only the structural work difference. Verification: mocked 89 suites / 1205 -> 89 / 1206 tests, 0 failures; e2e (real Postgres) 23 suites / 233 tests, 0 failures (one run failed 4 moderation tests only because I ran a second e2e concurrently against the shared test DB; both files pass alone); tsc, lint, nest build clean. PR opened, not merged.
- **Decision Log #350 (docs only, 2026-09-20) resolves by design: NO fixed-latency padding on `MessagingService.startConversation`.** Founder decision. The cheap signal (a structural extra query) was closed by PR #280; what remains needs high-volume repeated requests against one target account to extract a signal from ordinary network/DB jitter, and a permanent latency floor for every legitimate user isn't worth that. Not an oversight; don't re-litigate. Revisit only if counsel or a compliance requirement demands constant-time guarantees. Chain in one place: #347 -> PR #276 (enumeration fix) -> PR #280 (timing parity) -> #350; forward-pointer added on #347. PR opened, not merged.

- **`docs/legal-copy-draft-v0.3` (safeguarding-drafter work, 2026-09-20, docs only — no app/backend code) updates `docs/legal-copy-draft-tos-privacy-policy.md` from v0.2 to v0.3. Still NOT APPROVED; Decision Log #203 stays open, now "pending first counsel review of the v0.3 sections".** v0.2 (PRs #270/#271) applied counsel's 09/09/2026 "Approved with amendments" review; v0.3 adds only what four later-shipped safeguarding features require, drafted from the merged code, not a summary: (1) under-16 tier (PR #272, DL #346; ToS §3.5(a), PP §1.1/§1.2/§3/§7/§8); (2) adult-cannot-message-minor, enumeration-safe, residual timing jitter disclosed as an accepted limitation (PRs #273/#276/#280, DL #347/#350; ToS §3.5(b), PP §8); (3) consent audit trail now records screen version + coarse device type (PR #274, DL #348; PP §4.2, §9); (4) daily age-reclassification sweep, guardian email at 18, in-app notice at 16, ~24h window disclosed (PRs #275/#279/#281/#282, DL #349; ToS §3.6, PP §4.5, §9). A v0.3 changelog at the top lets counsel review the diff from v0.2. **Counsel-approved v0.2 wording was not silently rewritten**; four tensions are flagged in Part C rows 24–27 (under-16 403 confirms account existence; guardian authority at 18 vs the approved "guardian deletion request supersedes" sentence; looser "unverified accounts" wording in restricted-pending; the age log has no retention period). The v0.2 "[IN PROGRESS — not yet built]" notes in PP §4.2/§8 were our own scaffolding, not counsel language, and are replaced. Docx Decision Log #203 status cell updated. PR opened, not merged — Temi's call.
- **`docs/dpia-draft-refresh` (docs only, 2026-09-20) refreshes `docs/sprint-1-dpia-outline-draft.md` from v0.1 to v0.2, ready to send to counsel alongside legal-copy v0.3. Still NOT APPROVED; Decision Log #4 stays Open (forward-pointer added in the docx). This narrows the DPIA's factual gaps ahead of counsel's first full review and is NOT progress toward sign-off — the Section 6 sign-off block is deliberately empty.** Drafted from the merged code, not summaries. **R2 rewritten**: the v0.1 claim that nothing restricts who may message whom was false as of PRs #273/#276/#280 — now describes the adult-cannot-create-a-conversation-with-a-minor rule, under-16 exclusion, the shared 404 and equal DB work, and the residual gaps (timing jitter accepted, Decision Log #350; the under-16 recipient 403 still confirms account existence, which v0.2 states as a second residual; all rules key off self-declared DOB; ~24h age-flag lag). **R6 rewritten** as specific design choices for counsel: no IP is a deliberate permanent omission, the `Guardian` row (name/email/relationship) is hard-deleted at anonymization after being snapshotted into `ConsentAuditRecord`, which is itself hard-deleted on a fixed 6-month clock. **New R13** (under-16 tier) and **R14** (age-reclassification sweep). Also corrected because merged work had made them false: R1/R12 built-status, R4 (decline/withdraw/expiry built, PR #261), R5, §1.3/§1.5/§2.4. R3 and R7–R11 were NOT re-verified and are flagged unrefreshed in the doc. **Found while reading the code, not fixed here:** `docs/legal-copy-draft-tos-privacy-policy.md` v0.3 says the consent snapshot includes the guardian's relationship, but `anonymizeUser` does not snapshot it — one of the two must be reconciled before both go to counsel (DPIA §5 row 17). PR opened, not merged — Temi's call.
- **`figma/decision-log-cleanup-retouches` (figma-design-system, 2026-09-20) closes seven disclosed, non-blocking Figma gaps -- retouches to already-built screens only, no new screens, no code. Decision Log #53, #184, #185, #186, #312, #314 closed; #311 resolved-pending-code (companion `figma-to-code` PR wires the shared Header into the match-centre pages).** Figma writes done by the agent with direct shell access this session; docx Decision Log status cells updated in the same PR.
  - **#53 -- no change needed, verified.** The brief said `calendar 2` (`2365:2034`) was left out of the token pass; live audit shows it was already retrofitted by `sprint-2/admin-panel-structural-pass` (65 bound / 5 unbound -- the same disclosed 4 opacity-0 numerals + 1 hidden frosted panel; `calendar 1` closed by DL #178). Binding those 5 would force the token's alpha and reveal invisible days, so they stay. Entry marked Resolved with that finding.
  - **#184 -- header 7 nav layers renamed** `Group 833` -> `livesocre`, `Group 830` -> `blog` (`2841:4120`, `2841:4125`) to match header 4. Names only. header 4's own spellings (`livesocre`, `leaderboad`) were matched, not "corrected".
  - **#185/#186 -- Banter filter dialogs.** The opaque white full-bleed `Rectangle 134` scrim is now two stacked `color/icon/inactive` fills (Navigation Drawer precedent) -- on **three** frames, not two: the base `Bants homepage - search filter` (`2459:7671`) had the identical defect. The `Line 66` divider's unbound `#034694` stroke is rebound to `color/icon/inactive` on all three. **A mistake, caught and reverted in-pass:** my first divider sweep matched every unbound `#034694` paint and recoloured ~17 club-crest vectors + footer text per frame plus (via a revert that over-matched) 9 nav-icon vectors that were legitimately bound to that token; all restored, verified against untouched frame `2256:6802` (16 crest vectors + 1 footer text, unbound, opacity 1) and the nav icons re-bound. The `(c) 2022 Soccernity` footer text there is still off-palette blue with a stale year -- untouched, left for a footer pass. Residual risk: the 9 nav-icon vectors were rebound to a single-fill token paint on the assumption that was their original state.
  - **#312 -- 108 stat-bar tracks** (`#8f8f8f` @10%; 36 each on `640:3737`, `667:151`, `667:1511`) rebound to `color/icon/inactive`. Visual: slightly bluer/stronger track (navy 15% vs grey 10%).
  - **#314 -- crest overflow, root cause corrected.** Not a rendering-outside-bounds quirk: the crest group was **non-uniformly stretched** (same 132 vectors, 119 wide, 158-273 tall by frame), overrunning the team name at y=395. `XMLID_189_` resized to 119x158 at (337,196) on all 10 match-centre frames (verified within 1px against `Match Details`, the undistorted reference). **The Chelsea crest had the identical defect** (not named in the entry) -- inner group of `Group 118` set to 139x139 at (950,206) on the 9 frames that needed it.
  - **#311 -- Navbar on the 10 match-centre desktop frames** (`632:943`, `640:3737`, `667:151`, `667:1511`, `667:1952`, `756:11`, `756:6433`, `760:11533`, `6508:21059`, `6508:21321`): a 1440x90 `Navbar - header 4` (visible) + `Navbar - header 7` (hidden) at (0,0); the top 146px was empty so nothing moved. **Judgment call, flagged:** both auth variants on one frame (Blog PR #151 convention) rather than a Logged In/Out split (DL #169 later preferred the split for Blog) -- a split would be 10 more frames. Sports Hub is not login-gated and the shared Header switches at runtime, so this is design-only bookkeeping. **Closes the Figma half; code half is the companion PR.**
  - **Item 7 -- mobile homepage (`5543:7407`) brought up to the desktop retouch.** Eyebrow -> "WHERE THE WHOLE GAME LIVES", hero lede, pillar 3 ("The professional game, live." + body), closing sub, and the closing CTA label -> "Register your team" (same copy as desktop `5204:6728`, verified live). The single grassroots hero card rebuilt as the two-section split (Sports Hub Chelsea 3-1 Liverpool + Grassroots Ikoyi 2-1 Surulere + caption), cloned from the card's own bound nodes; 26 bound / 0 unbound paints in the card; the frame auto-hugged 6593 -> 6754px with no overlaps. Grassroots meta line wraps to two lines at 350px width (kept, not shortened). The section-label "Today's Fixtures" strip and everything else untouched, matching the desktop pass's scope.
  - **Not changed / still open:** the shared navbar avatar `IMAGE` fill debt; `header 7` desktop hidden variants pile up on the 10 frames; the footer `(c) 2022` / blue text on the Bants frames.
  - PR opened, not merged -- Temi's call after review.
- **`sprint-4/match-centre-navbar-wiring` (figma-to-code, 2026-09-20) is the code half of Decision Log #311 -- `apps/web` only, no production code change needed.** The match-centre route (`/sports-hub/matches/:matchId`) is already a direct `AppShell` child, so it already renders the shared `Header` (icon nav; `header 7` logged out / `header 4` logged in, switched at runtime per `sprint-2/navbar-phase-2-icon-nav-and-auth-state`) with no footer; the Figma navbar retrofit (PR #290) was design-only bookkeeping and the code already matched it. Added `router.test.tsx` cases proving it against the real route tree (logged out: Primary nav + Login, no footer; logged in: Primary nav, no Login). Nothing else on the page touched. PR opened, not merged.
- **`sprint-2/elevation-overlay-effect-style` (figma-design-system, 2026-09-21) resolves Decision Log #182 and #183 on the founder's go-ahead (2026-09-20) -- Figma design only, no app/backend code. THIS IS A VISIBLE DESIGN ADDITION, NOT A TOKEN REBIND: Temi should review the rendered result, not just the diff.** Report: `docs/sprint-2-elevation-overlay-effect-style-report.md`.
  - **New effect style `elevation/overlay`** (`S:fbda6d21...`): 3 stacked navy-derived drop shadows, all colours variable-bound -- ambient (y2 blur6), key (y8 blur24, reusing existing `color/shadow/elevated`), deep (y24 blur56). Two new `EFFECT_COLOR` variables (`color/shadow/overlay-ambient` Light navy 8% / Dark `#0D0F21` 30%; `color/shadow/overlay-key` Light navy 12% / Dark `#0D0F21` 55%) because one bound colour carries a single alpha for every layer, which is the flattening problem #182 named. Derived from existing navy / dark-page values; no new palette. `Soccernity Theme` is now 16 COLOR variables.
  - **Applied to panels that had no elevation:** Settings - Delete Role `Confirm Dialog` (`5404:7370`) and `Navigation Drawer - Mobile` `Panel` (`5870:10692`, inherited by its live instance). Screenshot-checked both: soft lift on the dialog, subtle bleed onto the scrim at the drawer edge.
  - **Left intact, per instruction:** the 4-layer `#130A2E` date-picker panels. **Flagged, not changed:** the Contest Rules dialogs (`6242:14768` / `6244:14768`) and Admin Open-the-final dialog (`6276:16201`) still use single-layer `elevation/menu`; they are dialogs and arguably belong on `elevation/overlay` -- founder call. The drawer shadow uses vertical offsets, not the horizontal leading-edge offset #183 suggested.
  - **#163(1) scrim token:** folded into this entry; no separate scrim fill token was created and existing stacked-fill scrims are untouched.
  - Decision Log #182/#183 flipped to Resolved; forward-pointer on #163. PR opened, not merged -- Temi verifies and merges.
- **`sprint-2/drawer-shadow-and-scrim-token` (figma-design-system, 2026-09-21) is two founder-requested Figma changes -- design only, no app/backend code. Decision Log #352 + #353; forward-pointers on #183 and #163.** Report: `docs/sprint-2-drawer-shadow-and-scrim-token-report.md`.
  - **Part 1 -- VISIBLE design change, review the render (Decision Log #352, supersedes the direction half of #183).** New effect style `elevation/overlay-right`: same 3 layers, radii and bound colours as `elevation/overlay`, offsets moved from y 2/8/24 to x 2/8/24 so the shadow casts right (the leading edge of the slide-in-from-left drawer). Applied to `Navigation Drawer -- Mobile` Panel `5870:10692` only. The Delete Role dialog (`5404:7370`) stays on the vertical `elevation/overlay`. No colour variable changed.
  - **Part 2 -- invisible refactor (Decision Log #353, resolves #163(1)).** New variable `color/overlay/scrim` (Light navy @ 27.75% = 1-0.85^2, the exact composite of the old two 15% fills; Dark `#0D0F21` @ 65%, continuing the dark ramp -- not visually verified, file is light-only). A file-wide search found exactly 8 stacked-fill scrims (drawer component + its instance override, both Contest Rules modals, Admin Open-the-final, three Bants filter dialogs); all retargeted to one bound fill, sampled pixels before/after differ by <=1/255. Paint convention: paint opacity = token alpha, bound to the variable (setting opacity 1 rendered solid navy -- caught by screenshot and corrected). **Not touched, flagged:** scrims bound to `brand/navy` with paint opacity 45% (Delete Role, 2 archived Contest frames) and 30% (Message actions menu) use different strengths. `Soccernity Theme` is now 17 COLOR variables.
  - PR opened, not merged -- Temi verifies and merges.
- **`sprint-3/nav-drawer-shadow-scrim-token` (figma-to-code, 2026-09-21) is the code half of Decision Log #352/#353 (PR #293) -- `apps/web` + `packages/shared` tokens only, no Figma/backend.** `packages/shared` colours gain four keys (`overlayScrim`, `shadowAmbient`, `shadowElevated`, `shadowKey`; Light navy 27.75/8/14/12%, Dark `#0D0F21` 65/30/45/55%, from the Figma variables) mapped in `applyTheme.ts` to `--sn-overlay-scrim` / `--sn-shadow-*`. `NavDrawer.css` `.sn-drawer__panel` gets the leading-edge `elevation/overlay-right` shadow (x offsets 2/8/24, blur 6/24/56, colours ambient/elevated/key) and its scrim, plus `ContestRulesModal.css`'s scrim, now use `var(--sn-overlay-scrim)` instead of the hardcoded `rgba(40, 46, 101, 0.28)` (grep confirmed these were the only two uses of that literal in `apps/*/src`). Other navy-alpha literals (Auth/Login/Home/Legal) and `EditProfileModal.css`'s different-strength scrim were left alone -- different values, not this pattern. **Verified headlessly** (real Chrome via puppeteer-core against the dev server, mocked API; "before" reproduced by injecting the old CSS values over the branch, not a separate build): Contest Rules modal before/after screenshots are pixel-identical (max diff 0); drawer scrim pixel unchanged (195,197,212) and the panel's right edge darkens (195,197,212 -> 172,175,196) as intended. Dark values are unused by the app (light-only). `npx tsc --noEmit`, lint clean; vitest 47 suites / 367 tests, 0 failures. PR opened, not merged -- Temi verifies and merges.
- **`sprint-1/coppa-card-verification` (backend-api + figma-to-code, 2026-09-21) adds a card-charge verification step to the guardian-consent flow for the under-13 US band -- a TECHNICAL CONTROL, NOT A LEGAL CONCLUSION. It does not make Soccernity "COPPA compliant"; Decision Log #4 stays open and this PR does not close it. Motivated by DPIA v0.3 §2.6 (PR #296), which found the email-link flow very likely does not meet COPPA's verifiable-parental-consent standard for that band.**
  - **Scope / branching**: in scope only when the declared DOB puts the child under 13 AND the self-declared country is US -- or the country is omitted (fail toward the stronger control). New `RegisterDto.countryCode` (ISO alpha-2, optional, self-declared, never IP-derived); `card-verification-policy.util.ts` decides once at registration and freezes the result in `Guardian.cardVerificationRequired` (a child who turns 13 while pending is not moved between paths). Everyone else (13-17 anywhere, under-13 with a declared non-US country, every pre-existing row via the column default `false`) keeps today's email-link-only flow untouched. Web captures the country on `GuardianDetailsStep` (required select; "Another country" = ISO `ZZ`, treated as non-US).
  - **Mechanism**: the email link stays REQUIRED (defense in depth); the card step is additive. `GuardianCardVerificationService` + three token-credentialed routes (`POST /auth/guardian-consent/verification`, `/card/intent`, `/card/complete`, all rate-limited, DTO accepts only `consentToken` so any card-shaped field is a 400). `confirmConsent` refuses with a 400 while `cardVerificationRequired && !cardVerifiedAt`. Stripe **PaymentIntent** (default 50 cents USD, `COPPA_VERIFICATION_AMOUNT_CENTS`, floor 50) with `receipt_email` = guardian, **refunded immediately** on success; the server re-reads the intent from Stripe and never trusts the client. A SetupIntent / $0 auth was rejected because it produces no discrete transaction or receipt, which is what the FTC method leans on. Refund failure keeps verification recorded and is retryable (`Guardian.cardRefundedAt`). Only a PaymentIntent REFERENCE id (`Guardian.stripePaymentIntentId`) is stored; card data goes browser -> Stripe Elements and never reaches or is logged by the server. New `src/payments/` (`CardVerificationGateway` seam + `StripeCardVerificationGateway`, "wired but inactive": no real keys => 503, in-scope guardians fail CLOSED).
  - **Audit trail (extends Decision Log #348)**: `Guardian.consentVerificationMethod` (`email_link` | `email_link_plus_card_charge`) + `consentVerificationAt`, written in the SAME `updateMany` as `consentTimestamp`, snapshotted by `AccountDeletionSweepService` into new `ConsentAuditRecord.verificationMethod` / `verificationAt`. The Stripe id and declared country are deliberately NOT snapshotted. Migration `20260921120000_add_coppa_card_verification` is purely additive. `CONSENT_SCREEN_VERSION` deliberately NOT bumped: the card panel lives in its own component, so the guardian-visible consent wording the version/tripwire covers is unchanged -- the verification-method column is what records the path.
  - **Still open / flagged**: no real Stripe account or keys exist (never run against live Stripe); no Figma frame exists for the card step or the country field (built plainly); PCI-DSS SAQ A operational follow-up for Temi; no reconciliation job for verified-but-unrefunded rows; the $0.50 in the panel copy is hardcoded while the amount is configurable; self-declared country is trivially bypassable by a child (inherent to self-declaration); Build Plan Decision Log entry not yet transcribed into the docx.
...
- This file has one real Figma variable collection: **`Soccernity Theme`**
  ...
  **Dark** (`5096:1`), **17** COLOR variables (`sprint-2/drawer-shadow-and-scrim-token` added `color/overlay/scrim`; `sprint-2/elevation-overlay-effect-style` added `color/shadow/overlay-ambient` and `color/shadow/overlay-key`, Decision Log #182) — Sprint D created the original
...
- **`sprint-6/leaderboard-read-rollup` (backend-api, 2026-09-14) builds
  ...
  the same founder-authorised-early-start pattern `admin`/`contest`/
  `grassroots`/`banter`/`messaging`/`notifications`/`community-groups`
  all already used. `services/api` only, no Figma/`apps/web` touched.
  Decision Log #292.** Report: `services/api/src/modules/leaderboard/README.md`
  (rewritten from its Sprint-6-placeholder state).
  - **New `LeaderboardModule`**, wired into `app.module.ts`: `GET
    /leaderboard?period=&cursor=&limit=` (`JwtAuthGuard`-only, Decision
    Log #129 — no logged-out view; no `@CurrentUser()` needed, the
    response carries no per-caller field, matching `GET /clubs`/`GET
    /banter-rooms`'s own "reading a directory isn't caller-scoped" shape,
    not `GET /posts/feed`'s), reading **exclusively** from the
    materialized `LeaderboardEntry` table — never aggregating
    `PointsLedgerEntry` on read, and a period the rollup hasn't touched
    yet returns an **empty page, not an error**. A new
    `LeaderboardRollupService` does the writing.
  - **Period format: ISO-8601 week (`"YYYY-Www"`, e.g. `"2026-W33"`),
    hand-rolled with plain `Date`/UTC methods in a new `iso-week.util.ts`**
    — confirmed via `grep -n "date-fns\|dayjs\|luxon\|moment" package.json`
    that this repo has zero date-library dependency, and
    `contest.service.ts`'s own `resolveRoundWindows` already sets the
    precedent of hand-rolling date arithmetic rather than adding a
    dependency for one module's math. Every computation is exclusively
    UTC (`Date.UTC`/`getUTC*`/`setUTC*`) — the exact DST-safety lesson
    `account-deletion-sweep.service.ts`'s own comment already documents
    (`setMonth` vs `setUTCMonth`). Provides both directions: date → period
    string, and period string → `[Monday 00:00:00 UTC, next Monday
    00:00:00 UTC)` boundary, plus a `getPreviousIsoWeekPeriod` that
    re-derives across an ISO-year boundary (week 1 of a year does not
    precede from "week 0" — it precedes from the *prior* ISO year's week
    52 or 53) rather than naively decrementing the week number. **All 31
    unit tests for this file were built against independently hand-derived
    reference dates** (the classic ISO-8601 `2008-12-29 → 2009-W01` /
    `2010-01-01 → 2009-W53` year-boundary examples, verified by plain
    weekday arithmetic anchored on the widely-documented fact that
    `2009-01-01` is a Thursday — not generated by running the
    implementation against its own output and asserting on that).
  - **Rollup mechanism: a `@Cron('0 */15 * * * *')` job** (no
    `EVERY_15_MINUTES` member exists on the installed `@nestjs/schedule`'s
    `CronExpression` enum — confirmed directly against
    `cron-expression.enum.d.ts`; this literal string matches its own
    `EVERY_10_MINUTES`/`EVERY_30_MINUTES` `"0 */N * * * *"` shape) in
    `LeaderboardRollupService`, reusing the already-globally-registered
    `ScheduleModule.forRoot()` — `AccountDeletionModule`'s own precedent,
    no new scheduling infra added. Each tick recomputes **both** the
    current ISO week **and** the immediately-preceding one — cheap
    insurance against the period-boundary edge case (a `PointsLedgerEntry`
    row awarded in the last few minutes of a week, read by a tick that
    lands just after the Monday-00:00-UTC rollover), instead of building
    separate finalization logic; a re-run on an already-settled period is
    a harmless idempotent no-op. `rollupPeriod(period)` runs **one** raw,
    parameterized `$queryRaw` aggregation — a CTE summing
    `PointsLedgerEntry.points` per `userId` (`INNER JOIN User`, filtered
    to `occurredAt` within the period's UTC boundary AND
    `User.accountStatus = 'active'`), then an outer `SELECT` computing
    `RANK() OVER (ORDER BY total DESC)` — Prisma's query builder cannot
    express a `GROUP BY` + window function, the same class of gap
    `clubs.service.ts`'s own raw-SQL precedent in this codebase exists
    for. Each aggregated row is then upserted into `LeaderboardEntry` via
    its existing `@@unique([userId, period])` key; a user with zero
    `PointsLedgerEntry` rows in the period is never materialized at all.
  - **Anti-gaming cap: `ENGAGEMENT_POINTS_CAP_PER_PERIOD = 100`**, in a
    new `leaderboard.constants.ts` — deliberately **not** added to
    `points/points.constants.ts` (that file is the write side's own
    concern; the cap is exclusively a read-side/rollup concern, per that
    file's own already-flagged follow-up). Applied via `LEAST(SUM(...),
    100)` in the raw SQL to **only** the summed contribution from
    `engagement_post`/`engagement_like`/`engagement_follow`, **before**
    adding the **uncapped** sum of `contest_weekly_win`/
    `contest_monthly_crown`. Contest points are never capped. Rationale:
    Decision Log #219's own ratio argument (a monthly crown of 250 should
    dominate ~83 posts of baseline engagement; a weekly win's smallest
    payout, 20 for 3rd place, should dominate ~7 posts) only holds if
    unbounded engagement volume can't out-rank it — 100 points is roughly
    2x a single 3rd-place weekly win and under half of the smallest
    1st-place weekly win (50), so a user cannot out-earn even one real
    Contest placement through engagement spam alone, while 100 points
    (~33 posts, ~100 likes, or ~100 follows within one ISO week)
    comfortably exceeds what a real single user does in a week of normal,
    unspammed use.
  - **Active-account exclusion (Decision Log #221), applied at BOTH
    stages**: (a) primary — the rollup's own raw SQL join filters
    `User.accountStatus = 'active'`, so a deactivated/`pending_deletion`
    user's row is simply never written/updated by a fresh rollup tick (an
    already-existing row from before they deactivated is left untouched,
    not deleted — no separate cleanup job, the stale row just sits there
    until read-time filtering hides it); (b) defensive — `GET
    /leaderboard`'s own read query **also** joins `User` and filters
    `accountStatus = 'active'` (mirroring `feed.service.ts`'s
    `ACTIVE_AUTHOR_POST_FILTER` / `users.service.ts`'s
    `ACTIVE_FOLLOW_ENTRY_FILTER` exactly), so a user who deactivates in
    the gap between two 15-minute ticks disappears from reads
    **immediately** rather than waiting up to 15 minutes — proven
    directly by an e2e test that writes a row while a user is active,
    deactivates them, and confirms `GET /leaderboard` hides them while the
    stale row still provably exists in Postgres.
  - **Pagination**: keyset (`rank` ASC, `userId` ASC tiebreak — `RANK()`
    deliberately gives tied users the same rank, Decision Log #61(c)),
    default page size 20 / max 50 per Section 5.5, same opaque-base64
    cursor convention as every other list endpoint in this codebase.
    Postgres's own default null ordering already puts `NULL` `rank`
    values last on an ascending sort (the opposite of MySQL's default),
    so no explicit `NULLS LAST` was needed — though in practice no
    materialized row ever has a null rank.
  - **Explicitly out of scope, per this task's own brief**: no on-read
    aggregation or backfill; no club-scoped filtering (`clubId` stays
    ignored — ties to the still-open Decision Log #74/#128
    represented-club question); no Competition board
    (Prediction/Commentary, Decision Log #72/#73); no `apps/web` or Figma
    change — `LeaderboardPage.tsx`'s Overall board conversion to this real
    endpoint is a separate follow-up PR, not part of this one (the
    existing Contest tab wired by `sprint-2/contest-posting-flow-to-code`
    is unaffected and untouched).
  - **Verification, all re-measured directly (stash/pop before-and-after,
    not estimated)**: mocked suite **56 → 60 suites / 796 → 854 tests, 0
    failures** (4 new suites — `iso-week.util.spec.ts` 31 tests,
    `leaderboard.service.spec.ts` 14 tests,
    `leaderboard-rollup.service.spec.ts` 9 tests,
    `leaderboard.controller.http.spec.ts` 7 tests — 58 new tests, no
    existing test changed; the `getIsoWeekPeriod`/`getPreviousIsoWeekPeriod`
    tests are the 31 hand-derived-fixture tests above). e2e suite (real
    Postgres via docker-compose, Docker Desktop confirmed already running
    in this session) **15 → 16 suites / 140 → 154 tests, 0 failures** —
    new `test/leaderboard.e2e-spec.ts`, 14 tests, calling
    `LeaderboardRollupService.rollupPeriod` directly (the same "call the
    underlying method directly with an explicit period/now" precedent
    `AccountDeletionSweepService`'s own e2e coverage set for its
    `@Cron()`-driven sweep) — proves the raw SQL's genuine correctness
    against a live engine: the cap, the mixed
    capped-engagement-plus-uncapped-contest case on the same user, both
    active-account exclusion stages, period isolation (a point earned in
    one ISO week never counts toward another), ties sharing a rank,
    rollup idempotency (three re-runs, one row, no duplicates), and full
    keyset pagination across 5 users with no gaps or duplicates. `nest
    build` and `npm run lint` both clean. **`User`/`Guardian` safeguarding
...
- **`sprint-2/settings-shell-to-code-account` (figma-to-code, 2026-09-22) — the code half of Decision Log #230/#249's Settings-family consolidation, `apps/web` only. Merged as PR #302; this bullet backfills it, since that PR itself never added a status bullet here — the exact drift this file's own "Keeping this file current" rule exists to prevent.** New shared `SettingsLayout`/`SettingsLayout.css` (Figma `Settings Shell` COMPONENT_SET `6339:16094` — desktop persistent rail, mobile `Settings Back Bar`/`Settings — Menu — Mobile` hub `6289:15068`), `SettingsLandingPage` (`/settings`), `SettingsMenuPage` (`/settings/menu`, mobile-only), `AccountOverviewPage` (`/settings/account` — Account Information/Club Representation/Change Password rendered disabled-with-note, matching the `PrivacySettingsPage.tsx` precedent; Deactivate/Delete are real, re-parented under `/settings/account/deactivate`/`delete` with old flat paths redirecting), and `SettingsSectionPlaceholder` (the honest "designed, not built in the app yet" state for Security/Notifications/Display, each still routed via a `section/*` wildcard at the time). `PrivacySettingsPage` re-parented under the shell (no more own heading/rail). `settingsSections.ts` is the five-section source of truth (`SETTINGS_SECTIONS`, `sectionForPath`) — Account and Privacy marked `built: true`, the other three `false` (the `built` flag itself isn't read by any routing logic; informational only). `SettingsShell.test.tsx` mounts the real route tree via `createMemoryRouter(routes)`. `apps/web` vitest 402 tests, 0 failures at merge.
- **`sprint-2/settings-shell-to-code-security` (figma-to-code, 2026-09-22) is PR 2 of the staged Settings set — the Security & Account Settings section (hub + Two-Factor Auth (SMS) leaf), stacking on PR 1 above.** Report in the PR description. **No live Figma MCP access this session** (the connector needs interactive re-authorization this session doesn't have) — content is reconstructed from this codebase's own already-merged Figma session reports (chiefly `docs/sprint-2-settings-family-consolidation-audit-report.md`, `docs/sprint-2-settings-duplicate-clusters-consolidation-report.md`, `docs/sprint-2-component-hygiene-toggles-nav-report.md`, `docs/sprint-2-settings-desktop-scaffolding-sweep-report.md`) rather than a fresh screenshot/context read — flagged in both page files' own header comments, not silently assumed current.
  - **`SecurityOverviewPage.tsx` (`/settings/security`)** — Figma `2926:8056` (desktop, renamed from "Settings — Security Overview") / `5695:8279` (mobile). Title "Security & Account Settings" (matching the rail label exactly, per this codebase's own established Account/Privacy convention of heading === rail label, and per Decision Log #230's PR6 fix aligning D9's on-screen heading to the rail); intro blurb "Manage your account's security and keep track of your account's usage." (the real D8-ported blurb, verbatim per the audit report); one row, "Two-factor authentication" → `/settings/security/two-factor`. The redundant one-row `Settings — Security & Account` intermediate (`2922:5143`/`5649:8074`, archived in Figma) is correctly not reproduced.
  - **`TwoFactorAuthPage.tsx` (`/settings/security/two-factor`)** — Figma `2926:8294` (desktop) / `5696:8213` (mobile). **No live 2FA/TOTP/SMS backend exists anywhere in `services/api`** (confirmed by grep across `services/api/src` for `two.?factor|totp|2fa` — zero matches). Two rows, "Text message" and "Authentication app" (the two real toggle labels this frame carries, confirmed via `sprint-2/component-hygiene-toggles-nav-report.md`'s own component-swap inventory) — both render as real, visible, **permanently-disabled** toggles with a plain note ("Not adjustable yet — two-factor authentication isn't available yet. This needs a backend security module first."), the same `PrivacySettingsPage.tsx` disabled-row-with-note discipline. **There is no Save/Submit action anywhere on this screen to fake a success path for** — the frame's own "Submit" button (`Frame 5926`) was confirmed, in a prior Figma session, to be dead hidden scaffolding with no live render, and was deleted there; nothing here pretends a change was ever saved. Row descriptions are a disclosed, non-literal gloss (no source records description copy for either row).
  - **`settingsSections.ts`**: `security.built` flipped `false → true`.
  - **`router.tsx`**: `{ path: "security/*", element: <SettingsSectionPlaceholder section="security" /> }` replaced with two explicit routes (`security`, `security/two-factor`), matching Account/Privacy's own no-wildcard convention for built sections — an unmatched deeper path (e.g. `/settings/security/foo`) now falls through to the real top-level 404 page, the same behaviour `/settings/account/foo` already has.
  - **`SettingsShell.test.tsx`**: `security` removed from the "Unbuilt sections" `it.each` table and its deep-unbuilt-path probe repointed to `/settings/notifications/foo` (still a real gap); new `describe("Security & Account Settings section")` block covers the hub's intro/row render, the rail marking Security active on both the hub and the leaf, the leaf rendering both rows permanently disabled with the note, and the no-session log-in prompt on both pages.
  - **Verified**: `npx tsc --noEmit`, `npm run lint`, `npm run build` all clean; `apps/web` vitest — real before/after via `git stash`, **49 suites / 399 tests, 0 failures → 49 suites / 402 tests, 0 failures** (3 new tests, no existing test removed — the deep-unbuilt-path probe was repointed, not deleted); a real dev-server smoke test on `/`, `/settings`, `/settings/security`, `/settings/security/two-factor`, `/settings/account`, `/settings/privacy` — all real HTTP 200, clean log. No real browser/Playwright check available in this environment — same verification ceiling as every prior `apps/web` figma-to-code PR.
  - Not merged — Temi's call after review.
- **`figma/trends-sidebar` (figma-to-code, 2026-09-26) builds the "Trends for you" desktop right-rail card on `/search` and wires it to the real, public `GET /trending` (`sprint-4/trending-topics-backend`) — `apps/web` only, no `services/api`/Figma changes.** Follows `figma/search-trending-shell` (PR #311, which left "Trending topics" as an empty `PlaceholderSlot`). Figma: desktop `2876:4628` — title `2876:4652`, rows `2876:4654-4667`, refresh `2876:4668`, three-dots `2876:4730-4742`, "See more" `2876:4711`. **The Figma has no single "Trends for you" group** — its parts are flat siblings of the page frame, so `get_design_context` on the card background (`2876:4647`) returns only a blank rectangle; built from the parent frame's metadata plus per-node context/screenshot instead. **Desktop only, by design:** the mobile frame (`5780:8581`) has no trends sidebar, so `TrendsForYou` is only mounted when `!isMobile` and never fetches on mobile (tested).
  - **New `api/trending.ts`** (public, no Authorization header — `TrendingController` has no guard) and **`pages/search/TrendsForYou.tsx`** (+ `trends-card*` styles appended to `SearchTrendingPage.css`; `trends-refresh.svg` / `trends-more.svg` exported from Figma). Rows render `#tag` + "N Posts" (singular "1 Post"). **The Figma's non-hashtag sample rows ("Ronaldo", "Manchester United") are not reproduced** — `GET /trending` returns hashtags only, nothing is fabricated.
  - **Live controls:** the refresh icon re-fetches; "See more" re-requests a larger top-N (`limit` 7 → 20, the endpoint has no cursor by design) and only shows when a full first page came back. **Deliberately disabled, not faked:** the per-row three-dots icon renders visibly disabled — no mute/hide-hashtag action exists anywhere (`search/README.md`). Rows are not links: `/search` has no `?q=` deep-link handling, so tapping a trend to search it is a follow-up.
  - **Verified:** `npx tsc --noEmit`, `npm run lint`, `npm run build` clean; `apps/web` vitest **51 suites / 446 tests, 0 failures** (before figure derived, not re-run: 50 suites / 439 tests, since no existing test was added or removed; new `TrendsForYou.test.tsx` +7; `SearchTrendingPage.test.tsx` updated — 5 → 4 placeholders, mocks `api/trending`, asserts no fetch on mobile). Not visually checked in a real browser (none available here).
  - PR #312 merged.
- **`figma/search-news-fixtures-panels` (figma-to-code, 2026-09-26) builds the "Trending News" (desktop left rail) and "Fixtures" (desktop right rail, under Trends) cards on `/search`, wired to existing endpoints — `apps/web` only, no new endpoints, no `services/api`/Figma changes.** Figma: desktop `2876:4628` — news title `2876:4712`, rows `2876:4715-4729`, clock icon `2876:4638`; fixtures title `2876:4653`, rows `2876:4871`. New `pages/search/TrendingNewsPanel.tsx` (public `GET /articles` via `api/blog.ts`) and `FixturesPanel.tsx` (public `GET /sports/fixtures?date=<today UTC>` via `api/sports.ts`, same `todayIso` convention as Sports Hub); both reuse the `.trends-card` chrome, mount desktop-only (nothing fetched on mobile), and replace two `PlaceholderSlot`s (Suggested and the video carousel remain placeholders).
  - **"Trending" = recency, a disclosed simplification** (code comment): no view/engagement tracking exists on articles (Decision Log #306), so the newest 3 published articles are shown in API order, cut client-side (`GET /articles` has no `limit`). Article has no image (Decision Log #334), so the thumbnail slot is a category-initial tile; blurb is the server `excerpt` (#333). Fixtures: first 7 of today's page, score for live/finished, kickoff time otherwise, monogram crest fallback; rows link to `/blog/:id` and `/sports-hub/matches/:id`. Both cards have live refresh, loading, empty and error states.
  - **Verified:** `npx tsc --noEmit`, `npm run lint`, `npm run build` clean; `apps/web` vitest **52 suites / 453 tests, 0 failures** (before 51 / 446; new `SidePanels.test.tsx` +7; `SearchTrendingPage.test.tsx` updated — 4 → 2 placeholders, asserts no news/fixtures fetch on mobile). Not visually checked in a real browser (none available here).
  - PR #313 merged.
- **`figma/suggested-people-panel` (figma-to-code, 2026-09-26) builds the "Suggested" people card (desktop left rail of `/search`, under Trending News) wired to the real `GET /users/suggested` — `apps/web` only, no `services/api`/Figma changes.** Figma: desktop `2876:4628`, card "Group 293" `2876:4744` (title `2876:4762`, rows `2876:4746-4773`, "See more" `2876:4751`). Desktop-only (no Suggested card in mobile frame `5780:8581`): nothing is mounted or fetched on mobile. New `getSuggestedUsers` in `api/users.ts` and `pages/search/SuggestedPeoplePanel.tsx`; **Follow/Following reuses the existing `followUser`/`unfollowUser` (POST/DELETE `/users/:id/follow`) — no new follow logic.** Suggestions are never already-followed users, so rows start "Follow" and flip in-session (both endpoints idempotent). The Figma photo avatars and `@handle` have no backing data (Decision Log #58), so rows render an initials avatar + display name only. The endpoint has no cursor, so "See more" re-requests a larger `limit` (3 → 10), the `TrendsForYou` approach; refresh re-fetches. The Following pill uses `#fff` on navy (no `--sn-text-on-navy` token, same disclosed exception as `accountActions.css`). The "Videos from Leaderboard" carousel was the last remaining `PlaceholderSlot` — built by `figma/leaderboard-videos-carousel` below.
  - **Verified:** `npx tsc --noEmit`, `npm run lint`, `npm run build` clean; `apps/web` vitest **53 suites / 461 tests, 0 failures** (before 52 / 453; new `SuggestedPeoplePanel.test.tsx` +8; `SearchTrendingPage.test.tsx` updated — 2 → 1 placeholder, asserts no suggestions fetch on mobile). Not visually checked in a real browser (none available here).
  - PR #314 merged.
- **`figma/leaderboard-videos-carousel` (figma-to-code, 2026-09-26) builds the "Videos from Leaderboard" carousel (desktop `/search` main column, Figma frame `2896:4770`) — the last `PlaceholderSlot` on that page (`PlaceholderSlot` and its CSS are now deleted). `apps/web` plus ONE additive `services/api` line.** Desktop-only; nothing mounted or fetched on mobile. New `pages/search/VideosCarousel.tsx` (+ `.css`, `play-circle.svg` exported from Figma), `lib/media.ts`, `recordPostView` in `api/feed.ts`.
  - **Real view counts, click-to-play only.** `POST /posts/:id/view` fires ONLY when a tile is clicked (never on render, scroll or impression — tested); the pill and player then show the count the server returns. `viewCount: 0` renders "0 views" (singular "1 view"); a failed view call keeps the last real value, nothing is incremented locally.
  - **One backend change, needed to show real counts at all:** `POST_SELECT` in `feed.service.ts` did not select `viewCount`, so no post payload carried it. Added `viewCount: true` (additive, no schema/migration; flows into `GET /posts/feed`, club/banter feeds, `POST /posts`). `FeedPost.viewCount` is now a required field on the web type.
  - **Video-vs-image: no existing distinction anywhere** (`PostCard` renders every `mediaUrls` entry as a plain link). Added a documented **v1 extension heuristic** (`.mp4/.webm/.mov/.m4v/.ogv`, query/fragment ignored) in `lib/media.ts`. Extensionless/signed URLs are not recognised.
  - **Disclosed data-source gap — "from Leaderboard" is the Figma title only.** No endpoint returns posts by leaderboard users and `GET /search` post results carry no `mediaUrls`; the only web-reachable source of `mediaUrls` is `GET /posts/feed` (caller's own + followed users' posts), so up to 5 pages are scanned client-side for video posts (max 12 tiles). A real leaderboard-scoped videos endpoint is a backend follow-up. Also: no media-upload endpoint exists for posts, so real video posts are rare today and the carousel will usually show its empty state ("No videos in your feed yet.").
  - Thumbnails are the video's own first frame (`<video preload="metadata" src="…#t=0.1">`), not Figma's photos; playback opens a modal `<video controls>` (Escape/backdrop closes).
  - **Verified:** `apps/web` `tsc`, lint, build clean; vitest **55 files / 473 tests, 0 failures** (before 461; new `VideosCarousel.test.tsx` +9, `lib/media.test.ts` +3, page test updated: 0 placeholders, no feed fetch on mobile). `services/api` `tsc` clean; feed/clubs/banter jest suites 8 suites / 218 tests, 0 failures (e2e not run). Not visually checked in a real browser (none available here).
  - PR opened, not merged — Temi's call after review.
